...
public interface IEntityQuery<TDomain>
{
    ...
    /// </summary>
    Task<IList<TDomain>> LoadAsync(CancellationToken ct = default);

    /// <summary>
    /// Execute the query against the data storage without attaching entities to the unit-of-work
    /// <param name="ct">CancellationToken which will be transitionally passed to underlying implementation logic </param>
    /// </summary>
    Task<IList<TDomain>> ExecuteQueryAsync(CancellationToken ct = default);
    ...
    /// <param name="ct">CancellationToken which will be transitionally passed to underlying implementation logic </param>
    /// </summary>
    Task ExecuteUpdateAsync(Func<IEntityUpdater<TDomain>, IEntityUpdater<TDomain>> configureEntityUpdater, CancellationToken ct = default);

    /// <summary>
    /// Run entities delete operation directly on a storage without loading entities to unit-of-work
    /// <param name="ct">CancellationToken which will be transitionally passed to underlying implementation logic </param>
    /// </summary>
    Task ExecuteDeleteAllAsync(CancellationToken ct = default);
}
